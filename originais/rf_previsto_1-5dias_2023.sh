...
# Risco de fogo previsto de 1km utilizando a temperatura, umidade relativa e a precipitação do GFS. Essas 
# simulações foram interpoladas a partir da resolução espacial de 50km (resolução original) para 1km.
...
data_previsao=`date +"%Y%m%d" -d "${data_final} 4 days "`"18"
#echo $data_previsao
#echo $data_modelo
#exit
...
#dirin_imerg="/mnt/vol_queimadas_1/produtos/meteorologia/dados_historicos/IMERG/netcdf"				# Dado observado de precipitação do IMERG (mm/dia).
...
dir_output_netcdf="/home/queimadas/INPE_FireRiskModel/data/output/2.2/RF_PREV/netcdf/${data_modelo}"	# Diretório de saída das previsões no formato NetCDF do RF previsto.
dir_output_tif="/home/queimadas/INPE_FireRiskModel/data/output/2.2/RF_PREV/tif/${data_modelo}"		# Diretório de saída das previsões no formato tif do RF previsto.
dir_output_fogograma="/home/queimadas/INPE_FireRiskModel/data/output/2.2/fogograma"
...
#arquivo_prec_imerg="PREC.IMERG.${data_inicial}00.nc"
...
do i = 0,nlat_veg-1
   ...
   do k = 0,nlon_veg-1
        ...
        if((.not.ismissing(pse_int_1km(0,i,k))).and.(.not.ismissing(pse_max(floattoint(mapa_veg(i,k))))) \  
            ...
            rb(0,i,k) = 0.9 ; Valor de rb será igual a 0.9.

        ; Caso contrário, calcula o rb de acordo com a equação abaixo.

        else 

            rb(0,i,k) = (0.9*(1+sin((A(floattoint(mapa_veg(i,k)))*pse_int_1km(0,i,k)-90)*(3.1416/180))))/2.0
...
EOF
	...
	echo "/home/queimadas/miniconda3/envs/ncl_stable/bin/ncl -npQ ${tmp}/rf.prev.${data_previsao}.ncl >> ${dir_log}/log.${data_modelo}.${data_previsao}" >> ${tmp}/paralelizar_RF_PREV.txt
...
${DIR_BIN}/parallel -j 20 -- < ${tmp}/paralelizar_RF_PREV.txt
...
# O 19 significa que são gerados 19 arquivos de previsão.
while [ ${arquivos} -ne 19 ]; do
	arquivos=$(ls ${dir_output_netcdf}/RF.PREV.* | wc -l)
...
# Cria um único arquivo com todos os horários de previsão para o fogograma.
${DIR_BIN}/cdo -s -O mergetime ${dir_output_netcdf}/RF.PREV.* ${dir_output_fogograma}/RF.PREV.${data_final}00.nc

# Enviando arquivos para o servidor geoserver:
/home/queimadas/INPE_FireRiskModel/scr/risco_fogo/RF_Previsto/envia_geoserv_tbrasilis.sh
